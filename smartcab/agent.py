...
class LearningAgent(Agent):
	...
	def get_maxQ(self, state):
		...
		maxQactions = filter(lambda k: actions[k] == maxQvalue, list(actions.keys()))
		maxQ = np.random.choice(maxQactions)

		self.log("actions", actions, "maxQ:", maxQ)
	...
	def createQ(self, state):
		...
		if self.learning and state not in self.Q.keys():
			self.Q[state] = dict(zip(Environment.valid_actions, np.zeros(shape=(len(Environment.valid_actions)))))

		self.log("created Q", self.Q[state])
	...
	def choose_action(self, state):
		...
		if not self.learning:
			...
			if random.random() < self.epsilon:
				action = random.choice(Environment.valid_actions)
				self.log("epsilon", self.epsilon, "choose random action....")
			else:
	...
	def learn(self, state, action, reward):
		...
		self.log("state", state, "action", action, "reward", reward)
		self.log("before learn", self.Q[state][action])

		self.Q[state][action] = (1 - self.alpha) * self.Q[state][action] + self.alpha * reward

		self.log("alpha", self.alpha, "reward", reward)
		self.log("after learn", self.Q[state][action])
...
def run():
	...
	agent = env.create_agent(LearningAgent, learning=True, debug=False, epsilon=1, alpha=0.6, a=0.9990)
